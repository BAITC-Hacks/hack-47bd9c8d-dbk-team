...
## Контроль поручений (Сценарий 2)

- `lib/commitments-store.ts` — поручения всех совещаний, с сохранением в MinIO
  (`results/commitments/index.json`). Переживают перезапуск контейнера.
  Адрес получателя, статус и отметки об отправке не затираются при повторном
  разборе той же записи.
- `lib/notifications.ts` — правила напоминаний: за `NOTIFY_SOON_DAYS` (по умолчанию 3)
  до срока и при просрочке, по одному разу на каждый повод. Отправка по HTTP
  (`MAIL_API_URL`, `MAIL_API_KEY`, `MAIL_FROM`) — подходит Resend, Mailgun,
  Postmark, MailChannels. Не настроено — письма пишутся в журнал
  (`results/commitments/outbox.json`) со статусом `pending`.
- `app/api/commitments` — `GET` список, `PATCH` адрес и статус.
- `app/api/commitments/notify` — `POST` проверка сроков и рассылка, `GET` журнал.
  Годится как цель для внешнего планировщика: повторный вызов не дублирует письма.

## Состояние переживает обновление страницы

`lib/active-meeting.ts` хранит в localStorage, какая встреча обрабатывается,
и время начала. На монтировании страница спрашивает `/api/meetings/[id]/status`
и возвращается в нужное состояние. Если сервер про встречу не знает — он мог
перезапуститься на полпути, — опрос продолжается ещё 15 минут, и только потом
состояние сбрасывается. Таймер считается от сохранённого начала.
