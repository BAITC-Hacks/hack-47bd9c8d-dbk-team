...
**Язык протокола** — переключатель `RU · KK · EN` в заголовке. Переводятся саммари и
поручения: перевод считается на своей модели, поэтому текст совещания не уходит наружу.
Первый перевод длинного протокола занимает до двух минут — рядом с переключателем видно
«переводим саммари и поручения». Дальше он берётся готовым: повторное переключение
мгновенное, и выбранный язык не сбрасывается при обновлении страницы. Транскрипт остаётся
на языке записи — это дословная запись сказанного. Выгрузка в DOCX и PDF идёт на том языке,
который выбран на экране.
