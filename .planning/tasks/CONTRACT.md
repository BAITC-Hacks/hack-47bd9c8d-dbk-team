...
Интерфейс и обработчик разошлись в именах топиков и форме сообщений. Между ними стоит
`services/adapter`: слушает `meetings.uploaded`, отдаёт обработчику
`AiMeetingCopilotRequest {messageId, files}`, ловит ответ, собирает `result.json`
из `transcript.json` и `summary.md` и публикует `meetings.ready`.

Поручения разбираются из таблицы в `summary.md`
(«Поручение | Ответственный | Срок | Классификация»), словесные сроки переводятся в ISO.
Ответственный вида `Speaker N` помечается `confidence: low` — привязка к человеку
не подтверждена.
...
```json
{
  ...
  "commitments":[{"id":"c1","assignee":"Гульмира Сериковна","assignee_speaker":"SPK_2",
                  ...
                  "classification":"Срочно · Договорная работа",
...
## Хранение поручений и напоминания

| Что | Где |
|---|---|
| Поручения всех совещаний | `results/commitments/index.json` в MinIO |
| Журнал отправленных писем | `results/commitments/outbox.json` |
| Почта провайдера | `MAIL_API_URL`, `MAIL_API_KEY`, `MAIL_FROM` в окружении интерфейса |
| Порог «скоро истекает» | `NOTIFY_SOON_DAYS`, по умолчанию 3 |

Поручение помечается отправленным только после успешной доставки: иначе настроенная
позже почта никогда не догонит старые поручения.
